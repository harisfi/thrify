...
if (isset($_POST['user']) && $_POST['user'] == "edit") {
    ...
    if (!empty($id) && !empty($id_a) && !empty($id_p) && !empty($nama) && !empty($tgl_lahir) && !empty($no_hp) && !empty($email) && !empty($status) && !empty($prov) && !empty($kota) && !empty($kec) && !empty($kel) && !empty($kd_pos) && !empty($provider) && !empty($no_rek)) {
        ...
        $query_u = "UPDATE tbl_user SET nama = '$nama', tanggal_lahir = '$tgl_lahir', no_hp = '$no_hp', email = '$email', verified = '$status', updated_at = current_timestamp(), deleted = null, deleted_at = null WHERE id = '$id'";
        $ret_u = mysqli_query($koneksi, $query_u);
        $jum_u = mysqli_affected_rows($koneksi);
        if ($jum_u > 0) {
            $query_a = "UPDATE tbl_alamat_user SET alamat1 = '$alamat1', alamat2 = '$alamat2', provinsi = '$prov', kota = '$kota', kecamatan = '$kec', kelurahan = '$kel', kode_pos = '$kd_pos', updated_at = current_timestamp(), deleted = null, deleted_at = null WHERE id = '$id_a'";
            $ret_a = mysqli_query($koneksi, $query_a);
            $jum_a = mysqli_affected_rows($koneksi);
            if ($jum_a > 0) {
                $query_p = "UPDATE tbl_pembayaran SET id_provider = '$provider', no_rek = '$no_rek', updated_at = current_timestamp(), deleted = null, deleted_at = null WHERE id = '$id_p'";
